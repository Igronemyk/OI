...
struct SuffixArray {
    ...
    /*
    void buildSA() {
        int nowRange = min(MAX_CHARSET_SIZE,length) - 1,*x = rank,*y = height,*cnt = new int[length];
        ...
        for(int i = 0;i < length;i++) {
            ...
            while(tmpPos + tmpHeight < length && i + tmpHeight < length && str[tmpPos + tmpHeight] == str[i + tmpHeight]) {
                tmpHeight++;
            }
